...
If TaIGa runs successfuly, it will print a message to the screen to informe so. There is no '-v' or '--verbose' option, so there's no way (yet) to disable TaIGas chit-chat. One way of working around this is to run TaIGa and send its output to a placeholder file, like so:
